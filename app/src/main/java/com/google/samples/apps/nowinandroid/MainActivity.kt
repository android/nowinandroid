    ...
    private fun logCompilationStatus() {
        ...
        Executors.newSingleThreadExecutor().submit {
            ...
            Log.d(
                TAG,
                when {
                    result.isCompiledWithProfile -> "ProfileInstaller: is compiled with profile2"
                    result.hasProfileEnqueuedForCompilation() -> "ProfileInstaller: " +
                        "Enqueued for compilation2"
                    else -> "Profile not compiled or enqueued2"
                },
            )
        }
    }
